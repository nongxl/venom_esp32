...
## 🛠️ 核心功能
*   **各向异性挤压与贴合物理重构 (v13.0 各向异性墙面挤压、自适应角落流动与爬行拖尾变细 / Anisotropic Wall Squeeze, Corner Flow & Crawling Tail)**：
    *   *各向异性贴墙挤压与压扁*：在渲染层 `drawBlob` 中，结合节点接触压力（`pressure`）与最近墙面的 2D 投影法线，在法向进行压扁（`SQUEEZE_NORMAL_DECAY = 0.62`）、在切向进行拉宽（`SQUEEZE_TANGENT_EXPAND = 1.35`），并与基于运动速度的拉伸进行加权混合，使 Metaball 在壁面上呈现出极致贴合的扁平接触面与弯月面过渡。
    *   *自适应角落流动填充*：当处于角落（CORNER）形态时，全身骨骼静息间距极致压缩至 35%，且刚度（stiffness）自适应衰减 72%（`CORNER_STIFFNESS_SCALE = 0.28`），使生物像液体一样“瘫软”并自然流动填充到墙角顶点。
    *   *爬行拖尾与拉伸变细*：主动爬行时，后部骨骼静息间距和 PBD 限制梯度延展至最大 1.75 倍（`CRAWL_REST_LENGTH_MULT`），并根据节点所受的拉伸张力（`tension`）进行自适应体积守恒变细处理（最细收缩 35%），在移动时拉出流畅、灵动的丝状液态拖尾，停止时平滑回缩融合。
    *   *天花板粘性提振与剥离回弹*：将天花板吸附强力提升 2.2 倍（`CEILING_SUCTION_BOOST`），并引入脱离瞬间基于 `RELEASE_KICK_WOBBLE = 4.5` 的释放抖动与回弹速度，赋予粘稠流体被强行撕离墙壁时的强韧弹性物理动感。
*   **表面贴合扁平化、接触压力场与心智符号融合机制 (v12.0 物理脱钩、符号融合与 Meniscus 圆角重构 / Surface Flattening, Symbol Fusion & Meniscus Corner)**：
    *   *物理 Z 轴锁死彻底脱钩*：在 `onWall` 计算中完全剔除 Z 轴深度干扰，使中后部悬空身体节点在前后方向实现 100% 自由摆动和恢复合拢，彻底终结了物理撕裂与悬空断开现象。
    *   *贴边 Meniscus 湿润圆角过渡*：各向异性切向拉伸微调至 0.88f（压缩 0.22f），多个节点在贴边碰撞时能通过势能场在边缘自然重叠，从而在贴墙切面两端自动融合出平滑、微翘的弯月面（Meniscus）圆滑液滴过渡，完全还原设计图的湿润质感。
    *   *液态几何符号势能融合*：大幅提升了液态心智符号（❓ 问号、❗ 感叹号、💖 爱心等）在势能场中的贡献系数（200f -> 270f），并注入 `55.0f * life` 的核心势能底限偏置，同时联动将背景阴影的起效阈值从 30 提升至 38。这使得符号粒子在空中完美融合为圆润饱满的纯黑色几何字形（外围带有柔和阴影），彻底根治了粗糙的“空心透明灰色噪点圈圈”问题。
...
*   **3D 透视渲染**：支持随设备倾斜而变化的**实时透视畸变**，增强实体的空间立体感。已对透视倾斜位移系数（`tiltFactor = 1.4f`）与近大远小底数（`220.0f`）进行精细收敛，消除深渊般过度拉伸的隧道感，使视觉深度与 20.0f 的超薄设计完美相符。
