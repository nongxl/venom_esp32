...
static constexpr float CUBE_D = 20.0f;  // 逻辑深度 (决定 3D 透视感)
...
// ============================================================================
static constexpr int   MAX_NODES         = 12;     // 身体节点总数: 8个是最佳平衡点
...
static constexpr float ADHESION_RANGE        = 15.0f; // [新增] 贴墙压力场敏感区间距离 (像素)
static constexpr float SPREAD_FORCE_MAG       = 2.2f;  // [新增] 切向扩散力强度，用于支持表面扁平展开

// ============================================================================
// 3.5. 贴合感重构新增参数
// ============================================================================
static constexpr float SQUEEZE_NORMAL_DECAY   = 0.62f;  // 贴墙时法向压扁系数（越小压得越扁）
static constexpr float SQUEEZE_TANGENT_EXPAND = 1.35f;  // 贴墙时切向拉宽系数
static constexpr float CORNER_STIFFNESS_SCALE = 0.28f;  // 角落形态下骨架刚度缩放（促进流动填充）
static constexpr float CRAWL_REST_LENGTH_MULT = 1.75f;  // 主动爬行时后部节段拉伸倍率（增强流体拖尾）
static constexpr float CEILING_SUCTION_BOOST   = 2.2f;   // 天花板吸附力额外提振系数
static constexpr float RELEASE_KICK_WOBBLE    = 4.5f;   // 剥离释放时的回弹抖动强度
...
static constexpr float BREATH_RANGE_BASE     = 0.16f; // 基础呼吸幅度
...
// ============================================================================
// 7. 震动与触觉反馈配置 (Haptic Feedback)
// ============================================================================
static constexpr int   PIN_MOTOR             = 0;     // 震动电机引脚 (StickS3 G0)
static constexpr float COLLISION_SPEED_MIN   = 1.2f;  // 触发震动的最小碰撞速度
static constexpr float COLLISION_SPEED_MAX   = 30.0f; // 达到最大震动的碰撞速度
static constexpr int   MOTOR_DUTY_MIN        = 45;    // 震动最小占空比 (防止电机死区不转)
static constexpr int   MOTOR_DUTY_MAX        = 180;   // 调优最大占空比，防止瞬时大电流拉低系统电压导致屏幕闪烁
static constexpr uint32_t IMPACT_DECAY_MS    = 120;   // 撞击震动基础衰减时间 (ms)
